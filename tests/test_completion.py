...
def test_complete_shows_stage_stem_refs(rfcs: Path) -> None:
    ...
    assert _vals(complete_rfc_id("")) == [
        "Idea-alpha-channels",
        "Idea-beta-buffer",
    ]
    assert _vals(complete_rfc_id("Idea")) == [
        "Idea-alpha-channels",
        "Idea-beta-buffer",
    ]
    assert _vals(complete_rfc_id("Ide")) == [
        "Idea-alpha-channels",
        "Idea-beta-buffer",
    ]
    assert _vals(complete_rfc_id("alpha")) == ["Idea-alpha-channels"]
    assert _vals(complete_rfc_id("Buf")) == ["Idea-beta-buffer"]
...
def test_complete_filters_irrelevant_prefixes(rfcs: Path) -> None:
    ...
    assert _vals(complete_rfc_id("e")) == []
    assert _vals(complete_rfc_id("Re")) == ["Research-beta-study"]
    assert _vals(complete_rfc_id("Acc")) == ["Accepted-gamma-done"]
    assert _vals(complete_rfc_id("Ide")) == ["Idea-alpha-channels"]
    assert _vals(complete_rfc_id("Gam")) == ["Accepted-gamma-done"]
    assert _vals(complete_rfc_id("Al")) == ["Idea-alpha-channels"]
    assert _vals(complete_rfc_id("xyz")) == []
...
def test_complete_type_prefix_ignores_title_word(rfcs: Path) -> None:
    ...
    assert _vals(complete_upgradeable_id("Ide")) == ["Idea-new-idea"]
    assert _vals(complete_upgradeable_id("New")) == [
        "Idea-new-idea",
        "Research-new-idea",
    ]
...
def test_complete_upgradeable_excludes_terminal(rfcs: Path) -> None:
    ...
    assert set(_vals(complete_upgradeable_id(""))) == {"Research-keep"}
...
def test_complete_accepted_only(rfcs: Path) -> None:
    ...
    assert _vals(complete_accepted_id("")) == ["Accepted-done"]
...
def test_upgrade_once_only(rfcs: Path) -> None:
    ...
    assert _vals(complete_upgradeable_id("")) == ["Research-once"]
